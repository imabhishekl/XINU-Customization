...
unsigned int    set_global_page_stuct();
...
nulluser()				/* babysit CPU when no one is home */
{
	...
	kprintf("clock %sabled\n", clkruns == 1?"en":"dis");
        ...
        /* Set the Page Frame and Page Directory */
        pdb = set_global_page_stuct();

        //kprintf("\npdb value is %08x",pdb);
        write_cr3(pdb); /* 1024*4096 */

        enable_paging();

        proctab[currpid].pdbr = pdb;
...
/* Routine to set the global page structure i.e. table and directory */
unsigned int set_global_page_stuct()
{
   int i,j;
   unsigned int pdb;

   pd_t *page_dir = (pd_t *)(FRAME0*NBPG); /* Maps to start of Free Frame memory location */
   pt_t *page_tab = (pt_t *)(PAGE_START_TAB*NBPG); /* Maps to start of the page table */

   pdb = (unsigned int)page_dir;

   /* Set the frame used to map */
   frm_tab[0].fr_status = FRM_MAPPED;
   frm_tab[0].fr_type = FR_DIR;
   frm_tab[0].fr_pid = currpid;
   frm_tab[0].fr_vpno = 0;
   ...
   for(i = 0;i < 4;i++)
   {
      ...
      page_dir->pd_acc = 0;
      page_dir->pd_fmb = 0;
      page_dir->pd_global = 1;
      page_dir->pd_base = (PAGE_START_TAB + i);/* Maps to the page frame */
      page_dir = page_dir + 1; /* Maps to start of Free Frame memory location */
      
      /* Set the frame used to map */
      frm_tab[1 + i].fr_status = FRM_MAPPED;
      frm_tab[1 + i].fr_type = FR_TBL;
      frm_tab[1 + i].fr_pid = currpid;
      frm_tab[1 + i].fr_vpno = 0;
      //kprintf("\nType<%d><%d>",(i + 1),frm_tab[1 + i].fr_type);
      ...
      for(j = 0; j < NFRAMES ; j++)
      {
         ...
         page_tab->pt_mbz = 0;
         page_tab->pt_dirty = 0;
         page_tab->pt_global = 1;
         page_tab->pt_avail = 0;
         page_tab->pt_base = i*NFRAMES + j;
         page_tab++;
      }
   }
   return (FRAME0*NBPG);
